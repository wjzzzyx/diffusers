...
class EulerSampler():
    ...
    def sample(
        ...
        if self.sampler == 'euler':
            sample = k_diffusion.sampling.sample_euler(
                denoiser, xi, sigmas, s_churn=0., s_tmin=0., s_tmax=float('inf'), s_noise=1., extra_args=extra_args
            )
        elif self.sampler == 'euler_ancestral':
            sample = k_diffusion.sampling.sample_euler_ancestral(
                denoiser, xi, sigmas, eta=1., s_noise=1., extra_args=extra_args
            )
    ...
    def sample_img2img(
        ...
        if self.sampler == 'euler':
            ...
            sample = k_diffusion.sampling.sample_euler_ancestral(
                denoiser, xi, sigma_sched, eta=1., s_noise=1., extra_args=extra_args
            )
