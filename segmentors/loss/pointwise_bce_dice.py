...
def get_uncertain_point_coords_with_randomness(logits, num_points, oversample_ratio, importance_sample_ratio):
    """
    ...
    Args:
        ...
        importance_sample_ratio (float): ratio of uncertainty-based important points
    Returns:
    ...
    point_logits = point_sample(logits, point_coords)
...
class PointwiseBCEDiceLoss(nn.Module):
    def __init__(self, oversample_ratio, importance_sample_ratio, num_points=112 * 112):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        with torch.no_grad():
            point_coords = get_uncertain_point_coords_with_randomness(
                pred, self.num_points, self.oversample_ratio, self.importance_sample_ratio
            )
            point_labels = point_sample(target.float(), point_coords).squeeze(1)
        point_logits = point_sample(pred, point_coords).squeeze(1)
    ...
    def bce_loss_func(self, logits, targets):
        """
        Args:
            logits: shape (batch, class, points)
            targets: shape (batch, class, points)
        """
        loss = F.binary_cross_entropy_with_logits(logits, targets, reduction='none')
        return loss.mean(1).sum() / len(logits)
