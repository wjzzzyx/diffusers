...
class TextDataset(object):
    ...
    def make_text_region(self, img, polygons):
        ...
        for idx, polygon in enumerate(polygons):
            ...
            dmp = ndimage.distance_transform_edt(inst_mask)  # distance transform
            maxdmp = np.max(dmp)
            ...
            proposal_points[idx, :, :] = \
                self.generate_proposal_point(dmp / (maxdmp+1e-3) >= self.th_b, self.num_points,
                                             self.approx_factor, jitter=self.jitter, distance=self.th_b * maxdmp)
    ...
    def get_test_data(self, image, polygons=None, image_id=None, image_path=None):
        ...
        points = np.zeros((self.max_annotation, 20, 2))
        length = np.zeros(self.max_annotation, dtype=int)
        label_tag = np.zeros(self.max_annotation, dtype=int)
