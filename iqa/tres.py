# Paper No-Reference Image Quality Assessment via Transformers, Relative Ranking, and Self-Consistency
# Code https://github.com/isalirezag/TReS/
...
class Net(nn.Module):
    def __init__(self,cfg, device):
        ...
        if cfg.network =='resnet50':
            resnet_modifyresnet = resnet50
            dim_modelt = 3840
            modelpretrain = resnet50(pretrained=True)

        elif cfg.network =='resnet34':
            resnet_modifyresnet = resnet34
            modelpretrain = resnet34(pretrained=True)
            dim_modelt = 960
            ...
            resnet_modifyresnet = resnet18
            modelpretrain = resnet18(pretrained=True)
            dim_modelt = 960
            self.L2pooling_l1 = L2pooling(channels=64)
            self.L2pooling_l2 = L2pooling(channels=128)
            self.L2pooling_l3 = L2pooling(channels=256)
